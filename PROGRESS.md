...
2. 录音闭环（60 秒、播放进度、损坏文件、旋转保留） — **已上 main**
3. 电子画册真实数据 + 长图/PDF 导出 — **已上 main**
4. 分享卡片真实照片 + 系统分享 — **已上 main**
5. 月年回顾 + 本地提醒 — **已上 main**
6. WorkManager 后台同步 — **已上 main**
7. 同步冲突解决 UI — **已上 main**
8. 设置 / 隐私 / 账号注销冷静期 — **已上 main**
9. 深色模式 + TalkBack / 大字体 — **本轮**
10. 桌面 Widget；正式阿里云短信/OSS（有密钥再做）

## 新一轮 10 项（2026-08-31）

1. 改密 + 本机已登录设备重置密码 — **已上 main**
2. 记录详情页 — **已上 main**
3. 照片压缩 / EXIF / 失败重试 — **已上 main**
4. 本地备份 / 恢复 — **已上 main**
5. 分享卡二维码 — **已上 main**
6. 纪念日与空间提醒 — **已上 main**
7. 草稿自动保存 — **已上 main**
8. Widget 一键记 — **已上 main**
9. 订阅档位 UI（支付后接） — **已上 main**
10. AI 小记助手（先规则） — **已上 main**

## 体验动效（2026-08-31）

统一页面转场、发布 GP 仪式缩短、心情回弹触感、记录页按钮状态、时间线吸顶日期、空状态、首次两步引导、植物阶段过渡、语音录制脉冲、详情看图。

## 未实现

- 正式阿里云短信/OSS（按产品改为账号密码，不再做短信）。
- 真实地图 SDK、订阅与 AI。
- 足迹列表已上（按地点名聚合，不接地图密钥）。
